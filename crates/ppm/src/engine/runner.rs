...
pub fn run_app(root: &Path, app_name_or_path: &str, user_args: &[String]) -> Result<i32, String> {
    ...
    let (exe_path, app_dir, extra_env, default_args) = if app_name_or_path.ends_with(".exe")
        ...
        || app_name_or_path.contains('/')
    {
        ...
        if let Some(deps) = &app_def.dependencies {
            for dep in deps {
                if let Some(dep_def) = manifests.apps.get(dep) {
                    if !dep_def.is_installed_for_arch(root, host_arch) {
                        return Err(format!(
                            "Required dependency '{}' is not installed for [{}]. Run 'ppm install {}' first.",
                            dep_def.name, host_arch.as_str(), dep
                        ));
                    }
                }
            }
        ...
        let exe = app_def.executable_for_arch(root, host_arch);
        let dir = app_def.app_dir_for_arch(root, host_arch);
        (exe, dir, app_def.env.clone(), app_def.default_args.clone())
    };
    ...
    // Auto-detect WebView2 engine if present under Apps/<host_arch>/WebView2
    let webview2_exe = root.join("Apps").join(host_arch.as_str()).join("WebView2").join("msedge.exe");
